...
def create_gcs_buckets(project_id, location="US"):
    ...
    for bucket_name in bucket_names:
        if not bucket_name:
            print(f"⚠️ Warning: A bucket name is not configured in YAML. Skipping.")
            continue
        ...
        try:
            if not bucket.exists():
                ...
                print(f"ℹ️ GCS Bucket already exists: {full_name}")
        except exceptions.Forbidden as e:
            print(f"❌ GCS Permission Denied for {full_name}: {e}")
        except Exception as e:
            print(f"⚠️ Failed to create GCS bucket {full_name}: {e}")
...
def create_bigquery_dataset(project_id, location="US"):
    ...
    if not dataset_name:
        print("⚠️ Warning: 'bigquery.dataset' not configured in YAML. Skipping BigQuery setup.")
        return
    ...
    try:
        ...
        print(f"ℹ️ BigQuery Dataset already exists: {dataset_id}")
        ...
        try:
            ...
            print(f"⚠️ Failed to create BigQuery dataset {dataset_id}: {e}")
